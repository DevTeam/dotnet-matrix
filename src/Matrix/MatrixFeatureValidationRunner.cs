...
public sealed class MatrixFeatureValidationRunner(
    MatrixModule module,
    ...
    IMatrixReportStore reportStore) : IMatrixRunner
{
    ...
    {
        var featureTypes = moduleAssembly.Value
            .GetTypes()
            .Select(type => (
                Type: type,
                Feature: type
        ...
        foreach (var library in libraries.OrderBy(item => item.Id, StringComparer.OrdinalIgnoreCase))
        {
            var version = library.Version is null ? string.Empty : $" {library.Version}";
            Console.WriteLine($"Validating {library.Name}{version}");
            foreach (var item in featureTypes)
            {
                ...
                if (method is null)
                {
                    ...
                    var reason = unavailable?.Reason
                                 ?? "No direct benchmark method or availability metadata exists.";
                    ...
                    if (instance is not null)
                    {
                        ...
                        catch (Exception cleanupException)
                        {
                            ...
                            Console.Error.WriteLine(
                                $"  {feature.Order:00} {feature.Name}: cleanup failed"
