...
{
    public const string Library = "Ninject";
    ...
    public static int Run()
    {
        ...
        kernel.Bind<IProbeSingleton>().To<ProbeSingleton>().InSingletonScope();
        kernel.Bind<IProbeTransient>().To<ProbeTransient>().InTransientScope();

        var s1 = kernel.Get<IProbeSingleton>();
        var s2 = kernel.Get<IProbeSingleton>();
        var t1 = kernel.Get<IProbeTransient>();
        var t2 = kernel.Get<IProbeTransient>();
